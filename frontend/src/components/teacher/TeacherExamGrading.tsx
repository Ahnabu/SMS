...
const TeacherExamGrading: React.FC = () => {
  ...
  const handleSaveGrades = async () => {
    ...
    try {
      setSaving(true);
      const response = await teacherApi.submitExamGrades({
        examId: selectedExam.examId,
        grades: gradesData,
      });

      if (response.data.success) {
        toast.success(`Grades submitted for ${gradesData.length} student(s)!`);
          ...
          >
            {saving ? (
              ...
              <>
                ...
                Save Grades ({gradedStudents}/{totalStudents})
              </>
            )}
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Exam Grading</h2>
          <p className="text-gray-600">Grade exams assigned to your subjects</p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm text-gray-500">Students to Grade</p>
                <p className="text-2xl font-bold text-gray-900">
                  {exams
                    .filter(e => e.status === 'completed')
                    .reduce((sum, e) => sum + (e.studentsCount - e.gradedCount), 0)}
                </p>
              </div>
            </div>
          </CardContent>
