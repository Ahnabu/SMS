...
export class CredentialGenerator {
  ...
  static parseStudentId(studentId: string): StudentIdComponents {
    if (!this.validateStudentIdFormat(studentId)) {
      throw new Error('Invalid student ID format. Expected: YYYYGGRRR (10 digits)');
    }
   ...
   */
  static async generateStudentCredentials(studentId: string): Promise<GeneratedCredentials> {
   ...
   */
  static async generateParentCredentials(studentId: string): Promise<GeneratedCredentials> {
   ...
   */
  static async checkUsernameAvailability(usernames: string[]): Promise<boolean> {
  ...
  static async generateStudentRegistration(
    ...
    const usernames = [credentials.student.username, credentials.parent.username];
