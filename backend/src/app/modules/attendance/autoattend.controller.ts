...
export const processAutoAttendEvent = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const school = await School.findOne({
      $or: orConditions,
      apiKey: apiKey,
    }).select("_id name slug schoolId apiKey isActive");
    ...
    // 4. Check for duplicate eventId (idempotency)
    ...
    // 5. Optional: Validate student exists in this school
